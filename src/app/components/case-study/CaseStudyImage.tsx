import { useState, ImgHTMLAttributes, CSSProperties } from "react";

interface CaseStudyImageProps extends Omit<ImgHTMLAttributes<HTMLImageElement>, "style"> {
  src: string;
  /** Friendly description shown in the empty state, e.g. "Seller Dashboard — Desktop" */
  label?: string;
  aspectRatio?: string;
  style?: CSSProperties;
}

export function CaseStudyImage({
  src,
  label,
  aspectRatio = "4/3",
  alt = "",
  style,
  ...rest
}: CaseStudyImageProps) {
  const [error, setError] = useState(false);

  if (error) {
    return (
      <div
        style={{
          aspectRatio,
          width: "100%",
          height: "100%",
          borderRadius: "inherit",
          border: "1.5px dashed rgba(212,175,55,0.35)",
          backgroundColor: "rgba(62,39,35,0.025)",
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
          justifyContent: "center",
          gap: 8,
          padding: 20,
          textAlign: "center",
          boxSizing: "border-box",
          ...style,
        }}
      >
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#D4AF37" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <rect x="3" y="3" width="18" height="18" rx="3" />
          <circle cx="9" cy="9" r="2" />
          <path d="M21 15l-5-5L5 21" />
        </svg>
        <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 11.5, fontWeight: 600, color: "#3E2723", opacity: 0.55, lineHeight: 1.45, maxWidth: 180 }}>
          {label || "Image coming soon"}
        </span>
        <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 9, color: "#3E2723", opacity: 0.32, wordBreak: "break-all" }}>
          {src.split("/").pop()}
        </span>
      </div>
    );
  }

  return (
    <img
      src={src}
      alt={alt}
      onError={() => setError(true)}
      style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", aspectRatio, ...style }}
      {...rest}
    />
  );
}
